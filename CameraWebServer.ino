...
void startDebugEndpoints() {
  ...
  // Software reset endpoint for camera reinitialization
  ...
  server.on("/health", HTTP_GET, [](AsyncWebServerRequest *request){
    bool camera_ok = (fb != nullptr && fb->len > 0);
    String status = camera_ok ? "Camera OK" : "Camera initializing...";
    request->send(camera_ok ? 200 : 503, "text/plain", status);
  });
}
